...
func (cq *CredentialQuery) sqlAll(ctx context.Context) ([]*Credential, error) {
	...
	if query := cq.withHolder; query != nil {
		ids := make([]uint, 0, len(nodes))
		nodeids := make(map[uint][]*Credential)
		for i := range nodes {
			if fk := nodes[i].user_credentials; fk != nil {
				ids = append(ids, *fk)
