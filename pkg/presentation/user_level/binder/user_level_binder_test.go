...
import (
	"fmt"
	binder2 "github.com/arpb2/C-3PO/pkg/presentation/user_level/binder"
	"testing"
...
func TestCreateBinder_RegistersRoutes(t *testing.T) {
	assert.NotNil(t, lookupController("GET", fmt.Sprintf("/users/:%s/levels/:%s", controller.ParamUserId, controller.ParamLevelId)))
	assert.NotNil(t, lookupController("PUT", fmt.Sprintf("/users/:%s/levels/:%s", controller.ParamUserId, controller.ParamLevelId)))
}
